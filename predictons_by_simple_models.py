...
# Colab / Python cell: fix notebook metadata widgets issue
...
# Путь к проблемному notebook — замените на свой путь
NOTEBOOK_PATH = "/content/your_notebook.ipynb"   # <- поменяйте здесь на фактический путь, например "/content/ml_direction.ipynb"

p = Path(NOTEBOOK_PATH)
if not p.exists():
    raise FileNotFoundError(f"{NOTEBOOK_PATH} not found. Укажите верный путь к .ipynb")

# бэкап
backup = p.with_suffix(".ipynb.bak")
...
if "widgets" in meta:
    ...
    # если widgets — словарь: попробуем добавить пустой 'state' там, где он отсутствует
    if isinstance(widgets, dict):
        for k, v in list(widgets.items()):
            if isinstance(v, dict):
                ...
                # неожиданная структура — удаляем эту запись
                print(f"  Unexpected widget entry for key {k!r}, removing it.")
                widgets.pop(k, None)
                fixed = True
        # записать обратно
        meta["widgets"] = widgets
    elif isinstance(widgets, list):
        # если это список — добавим state={} в элементы-словарях
        new_widgets = []
        for item in widgets:
            if isinstance(item, dict):
                ...
                # не словарь — игнорируем
                fixed = True
        meta["widgets"] = new_widgets
    else:
        # нестандартный тип — целесообразно удалить metadata.widgets
        print("metadata.widgets has unexpected type:", type(widgets), "-> removing metadata.widgets")
        meta.pop("widgets", None)
        fixed = True

    # если после попыток структуры всё ещё некорректно, удаляем metadata.widgets целиком
    # (это безопасно: потеряшь только состояние интерактивных виджетов, не код/данные)
    try:
        # check again minimal structure: each widget entry must be a dict and have 'state'
        ok = True
        if isinstance(meta.get("widgets"), dict):
            for v in meta["widgets"].values():
                if not isinstance(v, dict) or "state" not in v:
                    ok = False
                    break
        elif isinstance(meta.get("widgets"), list):
            for v in meta["widgets"]:
                if not isinstance(v, dict) or "state" not in v:
                    ok = False
                    break
        else:
            ok = False
        if not ok:
            print("Still invalid widget structure -> removing metadata.widgets entirely (safer).")
            meta.pop("widgets", None)
            fixed = True
    except Exception as e:
        print("Error validating widgets:", e)
        meta.pop("widgets", None)
        fixed = True
...
if fixed:
    ...
    print("Notebook fixed and saved:", p)
else:
    print("No changes required — notebook metadata.widgets already has 'state' entries.")